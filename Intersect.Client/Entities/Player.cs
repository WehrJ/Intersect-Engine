...
namespace Intersect.Client.Entities
{
    ...
    public class Player : Entity
    {
        ...
        public long[] MoveDirectionTimers = new long[4]; //VELODIA ADDITION FOR TAP MOVEMENT

        public string Faction; //VELODIA ADDITION FOR FACTIONS
            ...
            for (var i = 0; i < 4; i++)
                            {
                                if (i == Globals.Me.MoveDir)
                                    {
                                        //If we just started to change to a new direction then turn the player only (set the timer to now + 100ms)
                        ...
                        MoveDirectionTimers[i] = Globals.System.GetTimeMs() + 100;
                        Globals.Me.MoveDir = -1;
                                            }
        ...
        private void DrawNameAndLabels(Color textColor, Color borderColor, Color backgroundColor)
        {
            ...
            DrawFactionName(textColor, borderColor, backgroundColor);
                    }

        public virtual void DrawFactionName(Color textColor, Color borderColor = null, Color backgroundColor = null)
        {
            if (HideName || Faction == null || Faction.Trim().Length == 0)
            {
                return;
            }
...
var textSize = Graphics.Renderer.MeasureText(Faction, Graphics.EntityNameFont, 1);

var x = (int)Math.Ceiling(GetCenterPos().X);
var y = GetLabelLocation(LabelType.Faction);
...
Faction, Graphics.EntityNameFont, (int)(x - (int)Math.Ceiling(textSize.X / 2f)), (int)y, 1,
